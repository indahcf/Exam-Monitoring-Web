...
class PengawasModel extends Model
{
    ...
    public function getPengawasTersedia($tanggal, $jam_mulai, $jam_selesai)
    {
        ...
        // dd($pengawas_ada);
        return $this->db->table('pengawas')->select('*')->join('users', 'users.id=pengawas.id_user')->join('user_role', 'user_role.id_user=users.id')->whereNotIn('pengawas', $pengawas_ada)->where('user_role.id_role', 5)->Get()->getResultArray();
    }
